...
(function () {
  ...
  const frame = document.getElementById("wbLaunchFrame");
  if (!tab || !overlay || !frame) return;

  // Only show once logged in (a sidebar is present on every portal page), and only when we
  // aren't already looking at a whiteboard page ourselves.
  const sidebar = document.getElementById("sidebar");
  const alreadyOnWhiteboard = /\/whiteboard(s)?(\/|$)/.test(location.pathname);
  if (!sidebar || alreadyOnWhiteboard) return;

  // Admin sidebars render a "Students" list; student sidebars don't -- use that to pick
  // which board this tab should open. An admin opens their student picker; a student opens
  // their own board directly.
  const isAdmin = !!document.querySelector(".student-list");
  const targetUrl = isAdmin ? "/admin/whiteboards" : "/whiteboard";
  ...
  tab.addEventListener("click", () => {
    open = !open;
    if (open) {
      if (!frame.src) frame.src = targetUrl; // load lazily, only on first open
